...
allu_dat %>% group_by(tissue,toxin) %>% summarise(value = mean(value)) %>% 
  ggplot(aes(toxin,log10(value),fill=tissue))+
  geom_bar(stat = "identity", position = "dodge")+
  scale_fill_manual(values = c("lightblue","darkgreen"))

toxins<-read_csv("./data/toxins.csv",col_names = c("gene","gene_name","short","gene_type","trans","query"),skip = 1) 
toxin_families<-c("SVMP","SVSP","CTL","PLA2","LAAO","CRISP","VEGF","BPP","NGF")
d_t<-left_join(taiwan_merged_venom,toxins) %>% select(library,gene,tpm.x,tpm.y,est_counts,short) %>% rename(Toxin=short) %>% filter(Toxin %in% toxin_families)

d_t %>% na.omit() %>% filter(Toxin=="CRISP") %>% filter(est_counts >= 1) %>% ggplot(aes(log10(tpm.y), log10(tpm.x), color = library)) + geom_line(aes(group = gene), color = "grey") + geom_point() + theme_bw() + xlab("Venom gland gene expression (TPM)") + ylab("Venom mRNA level (TPM)") + guides(color = F) + ggtitle("Protobothrops mucrosquamatus") + theme(plot.title = element_text(face = "italic"))
```

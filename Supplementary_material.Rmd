...
meta_dat_plot<-pm_tpm %>% filter(gene %in% (modules %>% filter(colors == "turquoise"))$gene) %>% group_by(gene) %>% summarise(venom = mean(taiwan1:taiwan3),gland = mean(Pm_10:Pm_6)) %>% filter(venom >= 1) %>%gather(gene) %>% rename(Tissue = gene) %>% mutate(t = rep(seq(1:1502),2))


m1<-meta_dat_plot %>% ggplot(aes(t,log10(value+1),fill = Tissue))+geom_area()+
  scale_y_continuous(trans = 'sqrt')+
  scale_fill_jama()+ ylab("mean log TPM") + 
  xlab("metavenom genes")+
...
g<-c('107285132','107294779','107292484','107284495','107290246','107286125')
pm_tpm %>% filter(gene %in% (modules %>% filter(colors == "turquoise"))$gene) %>% 
  filter(gene %in% g) %>% group_by(gene) %>% summarise(venom = mean(taiwan1:taiwan3),gland = mean(Pm_10:Pm_6)) %>% 
  filter(venom >= 1)%>%gather(gene) %>% rename(Tissue = gene) %>% mutate(t = rep(seq(1:6),2)) %>% 
  ggplot(aes(t,log10(value+1),fill = Tissue))+
  geom_area()+
  scale_fill_jama()
...
m2<-muscle_merged %>% group_by(gene) %>% summarise(venom = mean(tpm.x),gland = mean(tpm.y)) %>% gather(gene) %>% mutate(t = rep(seq(1:554),2)) %>% rename(Tissue = gene) %>% ggplot(aes(t,log10(value+1),fill = Tissue))+geom_area()+
  scale_y_continuous(trans = 'sqrt')+
  scale_fill_jama()+ ylab("log TPM") + 
  xlab("muscle specific genes")+
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        aspect.ratio = 1/5)
